...
from icnet.data_util.cityscapes.cityscapes_utils import cityscapes_labels
...
def calc_log_frequency(classes, value=1.02):
    """Class balancing by ICNet method.
       prob = each_sum_pixel / each_sum_pixel.max()
       a = 1 / (log(1.02 + prob)).
    """
    class_freq = classes / classes.max() # ICNet is max, but ICNet is sum
    print(class_freq)
